...
export class AbstractSimpleMipsInstruction extends BaseInstruction {
    ...
    static extractArgs(word) {
        ...
        const immediateStr = bitString.slice(8);
        return [parseInt(immediateStr, 2).toString()]
    }
    ...
    resolvedOperand(system) {
        if (this.operandArg.startsWith("$")) {
            ...
            return Word.fromSignedIntValue(this.operandArg, system.addressSize)
        }
    }
    ...
    toMachineCode() {
        ...
        if (this.operandArg.startsWith("$")) {
            ...
            if (this.operandArg === "") {
                ...
                arg = parseInt(this.operandArg).toString(2)
            }
        }
...
export class OrInstruction extends AbstractSimpleMipsInstruction {
    ...
    executeOn(system) {
        const argWord = this.resolvedOperand(system);
        system.registers.$ACC.set(system.registers.$ACC.or(argWord));
    }
}
...
export class NotInstruction extends AbstractSimpleMipsInstruction {
    ...
    executeOn(system) {
        system.registers.$ACC.set(system.registers.$ACC.not());
    }
}
