...
import {BranchIfEqualInstruction, SimpleRISCInstructionFactory} from "@/architectures/simpleRISC/instructions.js";
...
test(`${mnemonic} instruction with immediate to string`, () => {
    const instruction = new BranchIfEqualInstruction(['202']);
...
test(`create ${mnemonic} instruction with immediate from mnemonic`, () => {
    const factory = new SimpleRISCInstructionFactory()
    const expectedInstruction = new BranchIfEqualInstruction(['202']);
...
test(`create ${mnemonic} instruction from machine code`, () => {
    ...
    const memory = [{
        address: 0,
        value: Word.fromString("000" + opCode + "1" + "00000001"),
    }];
