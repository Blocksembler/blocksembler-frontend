...
import {LeftShiftInstruction, SimpleRISCInstructionFactory} from "@/architectures/simpleRISC/instructions.js";
import {Word} from "@/architectures/system.js";
import {SimpleRISCEmulator} from "@/architectures/simpleRISC/system.js";

const mnemonic = "LSHIFT";
const opCode = "1001";

test(`${mnemonic} instruction to string`, () => {
    const instruction = new LeftShiftInstruction(['$1']);
    expect(instruction.toString()).toBe(`${mnemonic} $1`);
});

test(`${mnemonic} instruction with immediate to string`, () => {
    const instruction = new LeftShiftInstruction(['202']);
...
test(`test creating ${mnemonic} instruction from mnemonic`, () => {
    ...
    const expectedInstruction = new LeftShiftInstruction(['$1']);
...
test(`create ${mnemonic} instruction with immediate from mnemonic`, () => {
    const factory = new SimpleRISCInstructionFactory()
    const expectedInstruction = new LeftShiftInstruction(['202']);
...
test(`create ${mnemonic} instruction with immediate from machine code`, () => {
    const factory = new SimpleRISCInstructionFactory();
    const expectedInstruction = new LeftShiftInstruction(['255']);
...
test(`test execution of ${mnemonic} instruction`, () => {
    ...
    emulator.registers.$ACC = Word.fromString("0000000000000001")

    instruction.executeOn(emulator);

    expect(emulator.registers.$ACC.toBitString()).toBe("0000000000000010")
})