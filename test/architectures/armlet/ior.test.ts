...
import {ArmletInstructionFactory, IorImmediateInstruction, IorInstruction} from "@/architectures/armlet/instructions";
...
test("execute ior instruction with immediate", () => {
    ...
    mockSystem.memory[0] = {address: 0, value: Word.fromString(instruction.toMachineCode().slice(0, 16))};
    mockSystem.memory[1] = {address: 0, value: Word.fromString(instruction.toMachineCode().slice(16, 32))};
});
